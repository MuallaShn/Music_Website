import pandas as pd #pandan kullanarak verileri okumayı sağlayacağız
...
from sklearn.metrics.pairwise import cosine_similarity #2 vektörü karşılaştırarak benzlerlik n-bulur ona göre bir çıktı verir ,yukarıdaki metot ile orantılı çalışır
...
#burada metni kleimerle ayırıp kök bulma işlemi yapıyoruz ,sonra tüm kökü bulunmuş kelimeler birleştirilp daha sınra eğitmek için işimize yarayacak
...
df.to_csv("finaldatas.csv",index=False) # to_csv filename=finaldatas olarak ayarlandı kaydedilecek false olarak ayarlandı indekseler dosyaya kaydedilmesin diye

tfidvector = TfidfVectorizer(analyzer='word',stop_words='english')#burada metin içerisindeki kelimeleri tespit ediyoruz en çok tekrar edenleri analizliyoruz  
...
print(df[df['artist'] == 'Taylor Swift']['song'])# bir filtreleem işleminden geçirilir taylor swift içeren ksımları true alır ve işler daha sonra song kıyaslar ,seçilen sanatçı arafınan söylenen şarkıları çıkarırı
...
print(recommendation('Fearless')) #benzer şarkıları görmek için kullandık